  ...
  <div>
    <el-table :data="datalist">
      ...
      <el-table-column label="简介" prop="description" align="center" width="500px">
        <template #default="scope">
          <el-tooltip :content="scope.row.description" placement="top-start" effect="light" popper-class="tooltipStyle" :show-arrow="false">
            <span class="descriptionStyle">{{ scope.row.description }}</span>
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column label="删除时间" prop="last_mod_time" :formatter="dateFormat" align="center"> </el-table-column>
      <el-table-column label="操作" align="center">
        <template #default="scope">
          ...
          <el-button size="small" type="primary" @click="handleRecover(scope.row)" disabled>恢复</el-button>
...
const handleRecover = data => {
  ElMessageBox.confirm('恢复此文件至原有位置', '提示', {
...
const handleDelete = data => {
  ElMessageBox.confirm('该操作会彻底删除此文件', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
    draggable: true
